...
public class Quiz12 {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			sel = sc.nextInt();
			switch(sel) {
			case 1 : System.out.print("입실 호수 선택 : ");
					 room = sc.nextInt();
					 System.out.print("입실자 이름 : ");
					 hotel[(room/100)%10-1][(room%10)-1] = sc.next();
					 break;
			case 2 : 
				System.out.print("퇴실 호수 선택 : ");
					room = sc.nextInt();
					System.out.println(hotel[(room/100)%10-1][(room%10)-1]+"님 퇴실 되었습니다.");
					hotel[(room/100)%10-1][(room%10)-1] = null;
				break;
			...
			case 3 :
				for(int i=0; i<hotel.length; i++)
				{
					...
					{
						System.out.println((i+1)+"0"+ (j+1)+"호실 투숙객 : " + hotel[i][j]);
					}
				}
				break;
